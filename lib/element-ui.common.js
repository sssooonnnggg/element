...
/***/ (function(module, exports, __webpack_require__) {
	...
	exports.default = (_name$mixins$componen = {
	  ...
	  if (this.draggable) {
	    ...
	    this.drake.on("drop", function (el, target, source, sibling) {
	      ...
	      var siblingNode = _this.store.getNode(nodeId);
	      var node = _this.store.getNode(el.id);

	      if (!sibling) {
	        _this.$emit("node-insert-after", node.data, siblingNode.data);
	      } else {
	        _this.$emit("node-insert-before", node.data, siblingNode.data);
	      }
	    });
	...
	}, _name$mixins$componen.beforeDestroy = function beforeDestroy() {
	  if (this.draggable) {
	    this.drake.destroy();
	  }
